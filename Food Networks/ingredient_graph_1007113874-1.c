...
{
  if (source_ingredient == NULL) return -1;
...
{
  int idx = ingredient_index(source_ingredient);
  if (idx == -1) {
    return;
  }
  for (int i = 0; i < MAT_SIZE; i++) {
...
intNode *related_k_dist(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist)
{
  ...
  if (idx == -1) {
    ...
    // printf("idx %d\n", idx);
  for (int i = 0; i < MAT_SIZE; i++) {
    if (AdjMat[idx][i] > 0 && searchInt(h, i) == 0) {
      h = insertInt(h, i);
...
{
  intNode *s_head = NULL, *a_head =NULL;
    // printf("hihia\n");
  s_head = related_k_dist(s_head, source_ingredient, k_source, 0);
    // printf("hihib\n");
  a_head = related_k_dist(a_head, avoid, k_avoid, 0);
  int idx = ingredient_index(avoid);
  a_head = insertInt(a_head, idx);
    /*remove head*/
  while (s_head) {
    int is_not_inside = 1;
      for (intNode* now = a_head; now; now = now->next) {
        if (s_head->x == now->x) {
          intNode *tmp = s_head;
          s_head = s_head->next;
          free(tmp);
    ...
    if (is_not_inside) break;
  }
    // printf("hihi1\n");
    // print_ingredients(s_head);
    // printf("hihi4\n");
    // print_ingredients(a_head);
    // printf("hihi5\n");
  for (intNode* now = s_head; now && now->next; now = now->next) {
    for (intNode* av = a_head; av && now && now->next; av = av->next) {
      if (now->next->x == av->x) {
        intNode* tmp = now->next;
...
{
      int score[MAT_SIZE] = {0};
    int change_idx = 0;
    for (int i =0 ; i < 10; i++) {
        int idx = ingredient_index(recipe[i]);
        if (strcmp(recipe[i], to_change) == 0) change_idx = i;
        if (idx != -1) {
            for (int j = 0 ; j < MAT_SIZE; j++) {
                score[j] += AdjMat[idx][j];
            }
        }
    }
    int mx = -1;
    ...
    // recipe[change_idx] = ingredients[mx_idx];
    strcpy(recipe[change_idx], ingredients[mx_idx]);
  /*
   * *CRUNCHY!*
   * 
